...
class W4DModel(nn.Module):
    def __init__(self, 
        ...
        if config.use_cmd_embed:
            self._status_encoding = nn.Linear(4 + 2 + 2, config.tf_d_model)
        else:
            self._status_encoding = nn.Linear(2 + 2, config.tf_d_model)
        ...
        # # 增加一个网络，输入轨迹x,y坐标，能够预测轨迹的heading
        # self._heading_predictor = nn.Sequential(
        #     nn.Linear(self._num_poses * 2, config.tf_d_model),
        #     nn.ReLU(),
        #     nn.Linear(config.tf_d_model, config.tf_d_ffn),
        #     nn.ReLU(),
        #     nn.Linear(config.tf_d_ffn, self._num_poses)
        # )
    ...
    def forward_test(self, features) -> Dict[str, torch.Tensor]:
        ...
        if self._config.use_cmd_embed:
            ...
            ## 只使用车辆运动信息
            status_feature = status_feature[..., 4:]  # [bs, 4] 
        ...
        # 添加历史信息，进行时序特征融合
        if self.prev_keyval_feat is None:
            self.prev_keyval_feat = torch.zeros_like(keyval)

        keyval_final = torch.zeros_like(keyval)

        keyval_final = self._temp_decoder(keyval, self.prev_keyval_feat)  # [bs, 256+1, 256]

        self.prev_keyval_feat = keyval_final.detach()
    ...
    def forward_train(self, features) -> Dict[str, torch.Tensor]:
        ...
        keyval = keyval.clone() + self._keyval_embedding.weight[None, ...]  # [bs, 256+1, 256]
        ...
        ##==============================heading prediction========================================
        # traj_xy = target['trajectory'][:, :, :2].reshape(batch_size, -1)  # [bs, num_poses*2]
        ...
        if self._config.use_wm:
            ...
            wm_next_latent = self.wm_prediction(wm_keyval, trajectory['trajectory'])  # [bs, num_token, d_model]
            trajectory['pred_next_latent']=wm_next_latent
            trajectory['cur_latent']=wm_keyval
            return trajectory
        else:
            return trajectory
    ...
    def forward(self, object_queries, pred_cmd=None, cmd=None) -> Dict[str, torch.Tensor]:
        if self._num_mode:
            ...
            if self._num_mode == 20:
                ...
                use_pred = (cmd == 3)
                pred_idx = pred_cmd .to(poses.device).clamp(max=self._num_mode - 1)
                cmd_idx = cmd.to(poses.device).clamp(max=self._num_mode - 1)
                best_traj_idx = torch.where(use_pred, pred_idx, cmd_idx)
                batch_indices = torch.arange(poses.shape[0], device=poses.device)
                best_traj = poses[batch_indices, best_traj_idx]
                return {
                    "trajectory": best_traj,
