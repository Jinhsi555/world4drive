...
class W4DModel(nn.Module):
    def __init__(self, 
        ...
        # else:
        #     self._mode_embedding = nn.Embedding(self._num_mode * num_poses, config.tf_d_model)   # [num_mode * num_poses, d_model]
        ...
        if config.num_mode:
            self._query_embedding = nn.Embedding(self._num_mode * num_poses, config.tf_d_model)  # [num_mode * num_poses, d_model]
        else:
        ...
        if config.num_mode:
            self._trajectory_head = TrajectoryHead(num_poses, config.tf_d_ffn, config.tf_d_model, config.num_mode)
        else:
    ...
    def forward_test(self, features) -> Dict[str, torch.Tensor]:
        ...
        # =============================== 轨迹输出 ===========================================
        trajectory = self._trajectory_head(ego_query_out, cmd)
        return trajectory
    ...
    def forward_train(self, features) -> Dict[str, torch.Tensor]:
        ...
        ## mode query
        if self._config.num_mode:
            # mode_query = self._mode_embedding.weight[None, ...].repeat(batch_size, 1, 1)  # [bs, num_mode * num_poses, 256]
            # ego_query = ego_query.clone() + mode_query.clone()
            # ego_query = self._mode_encoder(ego_query, ego_query)  # [bs, num_mode * num_poses, 256]
            ...
            if self._num_mode == 20:
                ...
                mode_ref = self.waypoint_mode_ref.to(device).unsqueeze(0).repeat(batch_size, 1, 1, 1)  # [bs, num_mode, num_poses, 2]
            # mode_ref_feat_sin = self.gen_sineembed_for_position(mode_ref[..., -1, :])
            mode_ref_feat_sin = self.gen_sineembed_for_position(mode_ref) # [bs, num_mode, num_poses, 256]
            mode_ref_feat = self._mode_embedding(mode_ref_feat_sin)  # [bs, num_mode, num_poses, 256]

            ego_query = ego_query.clone() + mode_ref_feat.reshape(batch_size, -1, mode_ref_feat.shape[-1]).clone()  # [bs, num_mode * num_poses, 256]
    ...
    def forward(self, object_queries, cmd=None) -> Dict[str, torch.Tensor]:
        if self._num_mode:
            ...
            if self._num_mode == 20:
                ...
                batch_indices = torch.arange(poses.shape[0], device=poses.device)
                mode_per_cmd = self._num_mode // 3
                has_cmd3 = (cmd == 3)
                idx_offset = torch.arange(mode_per_cmd, device=poses.device)
                mode_indices = torch.zeros(
                    (poses.shape[0], mode_per_cmd), dtype=torch.long, device=poses.device
                )
                valid_mask = ~has_cmd3
                if valid_mask.any():
                    start_idx = (cmd[valid_mask] * mode_per_cmd).long()
                    mode_indices[valid_mask] = (start_idx[:, None] + idx_offset[None, :])
                if has_cmd3.any():  ## 如果命令3，需要使用全局最高分作为解
                    _, topk_idx = torch.topk(cls_score[has_cmd3], k=mode_per_cmd, dim=-1)
                    mode_indices[has_cmd3] = topk_idx
                expanded_idx = mode_indices[:, :, None, None].expand(-1, -1, self._num_poses, poses.size(-1))
                selected_poses = torch.gather(poses, 1, expanded_idx)
                selected_cls_score = torch.gather(cls_score, 1, mode_indices)
                best_traj_idx = torch.argmax(selected_cls_score, dim=-1)
                best_traj = selected_poses[batch_indices, best_traj_idx]
                # with torch.no_grad():
                #     full_best_idx = torch.argmax(cls_score, dim=-1)
                #     selected_global_idx = mode_indices[batch_indices, best_traj_idx]
                #     mismatch = full_best_idx != selected_global_idx
                #     if mismatch.any():
                #         print(
                #             "[TrajectoryHead] mode selection mismatch indices:",
                #             mismatch.nonzero(as_tuple=False).flatten().tolist(),
                #         )
                return {
                    "trajectory": best_traj,
                    "cls_logits": selected_cls_score,
                    "all_trajectories": selected_poses,
                    "all_cmd_trajectories": poses,
                    "all_cmd_cls_logits": cls_score,
                }
        else:
