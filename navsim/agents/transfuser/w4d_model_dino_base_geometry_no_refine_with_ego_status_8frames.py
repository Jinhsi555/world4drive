...
class W4DModel(nn.Module):
    ...
    def compute_loss(
        ...
        cmd_loss = cmd_cross_entropy_loss(predictions["cmd_pred"], predictions["cmd_gt"])
        velocity_loss = velocity_loss(predictions["velocity_pred"], predictions["velocity_gt"])
        acceleration_loss = acceleration_loss(predictions["acceleration_pred"], predictions["acceleration_gt"])
        loss_dict["ego_status_loss"] = (cmd_loss + velocity_loss + acceleration_loss) * self.ego_status_loss_weight
