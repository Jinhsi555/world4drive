...
class W4DModel(nn.Module):
    def __init__(self, 
        ...
        if self._config._use_mlp_ensemble:
            # 使用MLP进行轨迹集成
            ## 需要3个集成网络，分别集成2、3、4个历史帧
            self._mlp_ensemble_2 = nn.Sequential(
                nn.Linear((self._config.action_chunk_size -2) * 3, config.tf_d_ffn),
                nn.ReLU(),
                nn.Linear(config.tf_d_ffn, config.tf_d_model),
                nn.ReLU(),
                nn.Linear(config.tf_d_model, 3),
            )
            self._mlp_ensemble_3 = nn.Sequential(
                nn.Linear((self._config.action_chunk_size -1) * 3, config.tf_d_ffn),
                nn.ReLU(),
                nn.Linear(config.tf_d_ffn, config.tf_d_model),
                nn.ReLU(),
                nn.Linear(config.tf_d_model, 3),
            )
            self._mlp_ensemble_4 = nn.Sequential(
                nn.Linear(self._config.action_chunk_size * 3, config.tf_d_ffn),
                nn.ReLU(),
                nn.Linear(config.tf_d_ffn, config.tf_d_model),
                nn.ReLU(),
                nn.Linear(config.tf_d_model, 3),
    ...
    def _create_temporal_ensemble(self):
        ...
        return {
            ...
            'max_history': self._config.action_chunk_size - 1  # 最多使用的历史帧数
        }
    ...
    def _apply_temporal_ensemble(self, current_traj, history_trajs, current_step, 
        ...
        Args:
            current_traj: 当前步预测的轨迹 [bs, num_mode, chunk_size, 3]
        ...
        if len(related_trajs) < ensemble_config['min_history']:
            return current_traj
        # 如果使用网络集成，则不需要权重，使用网络来学习集成策略
        if self._config._use_mlp_ensemble:
            integrated_traj = self._integrate_trajectories_mlp(
                current_traj, related_trajs,
                batch_size, num_mode, chunk_size
            )
        else:
            # 计算时间集成权重
    ...
    def _integrate_trajectories_mlp(self, current_traj, related_trajs,batch_size, num_mode, chunk_size):
        """使用MLP进行轨迹集成"""
        # 与时间集成相似，但是将当前步的对应历史轨迹和当前轨迹拼接后输入MLP进行集成
        current_traj_reshaped = current_traj.reshape(
            batch_size, num_mode, chunk_size, 3)
        concat_trajs_list = []
        concat_trajs_list.append(current_traj_reshaped[:, :, 0, :])  # 当前轨迹的第一个waypoint
        for i, hist_traj in enumerate(related_trajs):
            hist_traj_reshaped = hist_traj.reshape(
                batch_size, num_mode, chunk_size, 3)
            hist_wp = hist_traj_reshaped[:, :, i + 1, :]  # 对应的历史轨迹waypoint
            concat_trajs_list.append(hist_wp)
        concat_trajs = torch.cat(concat_trajs_list, dim=-1)  # [bs, num_mode, (num_history+1)*3]
        # MLP集成
        num_history = len(related_trajs)
        if num_history == 1:
            integrated_first_waypoint = self._mlp_ensemble_2(concat_trajs)
        elif num_history == 2:
            integrated_first_waypoint = self._mlp_ensemble_3(concat_trajs)
        elif num_history == 3:
            integrated_first_waypoint = self._mlp_ensemble_4(concat_trajs)
        else:   
            integrated_first_waypoint = current_traj_reshaped[:, :, 0, :]  # 不进行集成，保持原样
        # 构建集成后的轨迹
        integrated_traj = current_traj_reshaped.clone()
        integrated_traj[:, :, 0, :] = integrated_first_waypoint
    ...
    def _integrate_trajectories(self, current_traj, related_trajs, weights, 
        ...
        weights = weights.to(current_traj_reshaped.dtype)
        first_wp = current_traj_reshaped[:, :, 0, :]
        pos_acc = weights[0] * first_wp[..., :2]
        sin_acc = weights[0] * torch.sin(first_wp[..., 2])
        cos_acc = weights[0] * torch.cos(first_wp[..., 2])
        ...
        for i, hist_traj in enumerate(related_trajs):
            if i < len(weights) - 1:  # 权重[0]已经用于当前预测
                ...
                # hist_index = num_his_traj - i
                hist_wp = hist_traj_reshaped[:, :, i + 1, :]
                pos_acc = pos_acc + weights[i + 1] * hist_wp[..., :2]
                sin_acc = sin_acc + weights[i + 1] * torch.sin(hist_wp[..., 2])
                cos_acc = cos_acc + weights[i + 1] * torch.cos(hist_wp[..., 2])

        integrated_first_waypoint = torch.zeros_like(first_wp)
        integrated_first_waypoint[..., :2] = pos_acc
        integrated_first_waypoint[..., 2] = torch.atan2(sin_acc, cos_acc)

        # current_traj_reshaped[:, :, 0, :] = integrated_first_waypoint
        # integrated_traj = current_traj_reshaped
        integrated_traj = current_traj_reshaped.clone()
        integrated_traj[:, :, 0, :] = integrated_first_waypoint
    ...
    def _delta_to_absolute(self, delta_traj: torch.Tensor, base_pose: torch.Tensor = None) -> torch.Tensor:
        # delta_traj: [bs, num_mode, num_poses, 3]，局部帧相对初始点的绝对位姿
        if base_pose is None:
            base_shape = delta_traj.shape[:-1] + (3,)
            base_pose = torch.zeros(
                base_shape, device=delta_traj.device, dtype=delta_traj.dtype
            )

        base_pose = base_pose.unsqueeze(-2)
        base_x = base_pose[..., 0:1]
        base_y = base_pose[..., 1:2]
        base_heading = base_pose[..., 2:3]

        local_pos = delta_traj[..., :2]
        local_heading = delta_traj[..., 2:3]

        cos_b = torch.cos(base_heading)
        sin_b = torch.sin(base_heading)

        global_x = base_x + local_pos[..., 0:1] * cos_b - local_pos[..., 1:2] * sin_b
        global_y = base_y + local_pos[..., 0:1] * sin_b + local_pos[..., 1:2] * cos_b
        global_heading = base_heading + local_heading
        global_heading = (global_heading + math.pi) % (2 * math.pi) - math.pi

        return torch.cat((global_x, global_y, global_heading), dim=-1)
    def _forward_core(self, features, is_test=False):
        ...
        if self._config.use_ar and self._num_mode == 4 and self._config.action_chunk_size > 1:
            ...
            mode_base_pose = torch.zeros(
                batch_size,
                self._num_mode,
                3,
                device=device,
                dtype=keyval_final.dtype,
            )
            ...
            for i in range(1, self._num_poses + 1):
                ...
                raw_chunk = last_trajectory['all_trajectories']  # [bs, num_mode, chunk_size, 3]
                local_action_first = raw_chunk[:, :, 0, :]

                current_base_pose = mode_base_pose
                abs_chunk = self._delta_to_absolute(raw_chunk, base_pose=current_base_pose)
                # abs_chunk_flat = abs_chunk.reshape(batch_size, self._num_mode * self._config.action_chunk_size, 3)
                ## TODO: 或许可以尝试使用MLP网络去学习自适应的集成策略
                if i > 1 and len(all_cmd_temp_traj) > 0:
                    abs_chunk = self._apply_temporal_ensemble(
                        abs_chunk, all_cmd_temp_traj, i,
                        batch_size, self._num_mode, self._config.action_chunk_size
                    )

                abs_chunk = abs_chunk.reshape(batch_size, self._num_mode, self._config.action_chunk_size, 3)
                mode_base_pose = abs_chunk[:, :, -1, :]

                cur_action_first_origin = abs_chunk[:, :, 0, :]
                all_step_ensembled_traj.append(cur_action_first_origin)
                all_cmd_temp_traj.append(abs_chunk)
                cur_pred_next_latent = []
                if self._config.use_ar_wm:
                    for wm_idx in range(self._num_mode):
                        cur_cmd_wm_next_latent = self.ar_chunk_wm_prediction(kv, local_action_first[:, wm_idx, :], i)
                ...
                if is_test:
                    ...
                    all_next_latent.append(true_cmd_wm_next_latent)
                    kv = true_cmd_wm_next_latent
            ...
            ensembled_traj = torch.stack(all_step_ensembled_traj, dim=2).reshape(batch_size, self._num_mode, self._num_poses, 3)
            # ensembled_traj[..., 2] = torch.atan2(torch.sin(ensembled_traj[..., 2]), torch.cos(ensembled_traj[..., 2]))
            heading = torch.atan2(
                torch.sin(ensembled_traj[..., 2]), torch.cos(ensembled_traj[..., 2])
            )
            ensembled_traj = torch.cat(
                [ensembled_traj[..., :2], heading.unsqueeze(-1)],
                dim=-1,
            )
