...
class W4DModel(nn.Module):
    def __init__(self, 
        ...
        # AR的逐步残差预测解码器
        if self._config.use_ar and self._num_mode == 3:
            for i in range(self._num_poses):
    ...
    def _forward_core(self, features):
        """
        ...
        if self._config.use_ar and self._num_mode == 3:
            ...
            for i in range(1, self._num_poses + 1):
                ...
                last_trajectory = self._trajectory_head.forward_ar(out, pred_cmd=cmd_pred, cmd=cmd)
            ...
            trajectory = self._trajectory_head(ego_query_out, cmd_pred, cmd=cmd)
    ...
    def forward_test(self, features) -> Dict[str, torch.Tensor]:
        ...
        trajectory, _ = self._forward_core(features)
        return trajectory
...
class TrajectoryHead(nn.Module):
    ...
    def forward_ar(self, object_queries, pred_cmd=None, cmd=None) -> Dict[str, torch.Tensor]:
        ...
        if self._num_mode == 3 and pred_cmd is not None:
