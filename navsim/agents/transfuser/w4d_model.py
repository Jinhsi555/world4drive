...
class W4DModel(nn.Module):
    def __init__(self, 
        ...
        ############################################多模态相关初始化########################################
        # 增加多模态轨迹嵌入
        self._num_mode = config.num_mode if hasattr(config, 'num_mode') else 8
        ...
        ## 增加多模态注意力层来强化多模态信息
        # if config.num_mode:
        #     self._mode_encoder_layer = nn.TransformerDecoderLayer(
        ...
        #         batch_first=True,
        #     )
        #     self._mode_encoder = nn.TransformerDecoder(self._mode_encoder_layer, 1)
    ...
    def forward_test(self, features) -> Dict[str, torch.Tensor]:
        ...
        # =============================== 轨迹输出 ===========================================
        trajectory = self._trajectory_head(ego_query_out)
        return trajectory
...
class TrajectoryHead(nn.Module):
    ...
    def forward(self, object_queries) -> Dict[str, torch.Tensor]:
        if self._num_mode:
            ...
            best_traj_idx = torch.argmax(cls_score, dim=-1) # [bs]
            best_traj = poses[torch.arange(poses.shape[0]), best_traj_idx]  # [bs, num_poses, 3]
