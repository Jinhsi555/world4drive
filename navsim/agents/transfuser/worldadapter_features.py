...
def dump_feature_target_to_pickle(path: Path, data_dict: Dict[str, torch.Tensor]) -> None:
    ...
    data_dict_cpu = {
        k: v.float().detach().cpu() if isinstance(v, torch.Tensor) else v 
...
class WorldAdapterFeatureBuilder(AbstractFeatureBuilder):
    ...
    def get_geometry_feature(self, camera_inputs: List[np.ndarray]) -> torch.Tensor:
        image_inputs, _, intrinsics, extrinsics = camera_inputs
    ...
    def compute_features(self, agent_input: AgentInput, scene: Scene, feature_cache_path: Path) -> Dict[str, torch.Tensor]:
        ...
        _, image_path, _, _ = camera_inputs
        ...
        for key, value in final_features.items():
            if 'camera_feature' in key:
                save_dict[value] = {
                    ...
                    'geometry_feature': feature_cache_path / value / f"geometry_feature.npy"
                }
        
        # 存储二级索引
        for token_name, cache_feature in save_dict.items():
            os.makedirs(feature_cache_path / token_name, exist_ok=True)
            save_path = feature_cache_path / token_name / f"{token_name}.gz"
        ...
        # geometry feature 三级存储
        for key, value in final_features.items():
            if 'camera_feature' in key:
                geometry_feature_path = feature_cache_path / value / f"geometry_feature.npy"
                if geometry_feature_path.exists():
                    print(f"跳过已存在的文件: {geometry_feature_path}")
                    continue
                geometry_feature = features[key]['geometry_feature']
                save_geometry_feature_to_npy(geometry_feature_path, geometry_feature)
                while True:
                    try:
                        check_data = load_geometry_feature_from_npy(geometry_feature_path)
                        break
                    except Exception as e:
                        print(f"Error loading {geometry_feature_path}: {e}, cache again...")
                        save_geometry_feature_to_npy(geometry_feature_path, geometry_feature)
    ...
    def _get_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        for camera_token in ['cam_l0', 'cam_f0', 'cam_r0']:
            ...
            image_path_list.append(camera.camera_path)
...
class WorldAdapterTargetBuilder(AbstractTargetBuilder):
