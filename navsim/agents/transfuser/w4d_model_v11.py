    ...
    def _forward_core(self, features, is_test=False):
        ...
        if self._config.use_ar and self._num_mode == 4:
            ...
            for i in range(1, self._num_poses + 1):
                ...
                if self._config.use_ar_wm and not is_test:
                    ## 这里如果使用kv来替换wm_next_latent的话，就是类似闭环的效果设计
                    wm_next_latent = self.ar_wm_prediction(keyval_final, last_trajectory['trajectory'], i)
                    ...
                    keyval_next = keyval_next.clone() + self._keyval_embedding.weight[None, ...]
            ...
            if self._config.use_ar_wm:
                trajectory['all_pred_next_latent'] = all_pred_next_latent  # list of [bs, num_token, d_model]
    ...
    def compute_loss(
        ...
        if self._config.use_ar_wm and 'all_pred_next_latent' in predictions:
            ...
            for i, pred_latent in enumerate(all_pred_next_latent):
                ...
                loss_dict[f'ar_wm_loss_step_{i+1}'] = wm_loss_ar * self.wm_loss_weight * (0.9 ** i)
