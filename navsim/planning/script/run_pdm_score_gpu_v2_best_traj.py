...
from navsim.evaluate.pdm_score import pdm_score, pdm_score_batch_trajectories, pdm_score_best_trajectories
...
def run_pdm_score_wo_inference(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_one):
        ...
        try:
            ...
            trajectory = model_trajectory[token]['all_trajectory']

            score_row_stage_one, ego_simulated_states, best_trajectory = pdm_score_best_trajectories(
            ...
            end_pose = StateSE2(
                x=best_trajectory.poses[-1, 0],
                y=best_trajectory.poses[-1, 1],
                heading=best_trajectory.poses[-1, 2],
            )
    ...
    for idx, (token) in enumerate(tokens_to_evaluate_stage_two):
        ...
        try:
            ...
            score_row_stage_two, ego_simulated_states, best_trajectory = pdm_score_best_trajectories(
                metric_cache=metric_cache,
                vocab_trajectories=trajectory,
                future_sampling=simulator.proposal_sampling,
                simulator=simulator,
