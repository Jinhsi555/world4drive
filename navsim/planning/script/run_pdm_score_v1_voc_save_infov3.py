...
def run_pdm_score(
    ...
    ## 只取10个tokens做测试
    # tokens_to_evaluate = tokens_to_evaluate[:3]
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            lat_dists = np.abs(model_trajectory_sub[:, -1, 1] - target_gt_traj[-1, 1])  # 横向距离
            lon_dists = np.abs(model_trajectory_sub[:, -1, 0] - target_gt_traj[-1, 0])  # 纵向距离
