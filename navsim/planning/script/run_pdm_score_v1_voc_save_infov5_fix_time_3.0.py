...
def run_pdm_score(
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            # 开始筛选,使用8s轨迹的终点信息去筛选
            target_gt_traj = model_trajectory[0]  # gt轨迹(8,3)
            ...
            lat_dists = np.abs(model_trajectory[:, -1, 1] - target_gt_traj[-1, 1])  # 横向距离
            lon_dists = np.abs(model_trajectory[:, -1, 0] - target_gt_traj[-1, 0])  # 纵向距离
            mask_lat = lat_dists <= lat_dist_threshold
            mask_lon = lon_dists <= lon_dist_threshold

            # B 计算角度偏差，选取终点方向与gt轨迹终点方向偏差在一定范围内的轨迹
            target_yaw = model_trajectory[0, -1, 2]
            ## 计算角度差后把弧度制转成角度制
            yaw_diffs = np.abs(model_trajectory[:, -1, 2] - target_yaw)
...
def main(cfg: DictConfig) -> None:
    ...
    # 构建数据点用于并行处理，只处理之前漏掉的log文件
    data_info = load_appended_pickle(Path('/mnt/parallel_ssd/home/zdhs0121/Driving/navsim_workspace/Epona/exp/navtrain_score_timehorizon_3.0.pkl'))
    data_points = []
    for log_file, tokens_list in scene_loader.get_tokens_list_per_log().items():
        # 检查该 log_file 是否已经处理完毕
        all_tokens_processed = all(
            token in data_info for token in tokens_list
        )
        if all_tokens_processed:
            logger.info(f"Skipping already processed log file: {log_file}")
            continue
        else:
            logger.info(f"Adding log file for processing: {log_file}")
            # 添加到 data_points 列表
            data_points.append(
                {
                    "cfg": cfg,
    ...
    print(f"Total log files to process: {len(data_points)}")
    # data_points = [
    #     {
    #         "cfg": cfg,
    #         "log_file": log_file,
