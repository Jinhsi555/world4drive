...
def rel2abs_traj_transform(rel_traj: list):
    """
    列表中是8个轨迹,每个轨迹都是8个时间步的坐标,每个轨迹的第一步都代表相对于上一个时间步的x,y和yaw变化,其中yaw是角度制需要转换成弧度制
    我们需要把8个轨迹的第一步取出来,形成一个新的轨迹,有8个时间步,每个时间步是相对于上一个时间步的变化,我们需要把这个相对的轨迹转为相对于起点的绝对轨迹,
    第一个轨迹是相对于起点的变化,需要把这个8个时间步转化为相对于起点的绝对轨迹
    """
    abs_traj = []
    curr_x, curr_y, curr_yaw = 0.0, 0.0, 0.0
    for step in range(len(rel_traj[0])):
        delta_x = rel_traj[step][0,0].item()
        delta_y = rel_traj[step][0,1].item()
        delta_yaw = np.deg2rad(rel_traj[step][0,2].item())  # 转为弧度制

        # 计算全局坐标系下的增量
        global_delta_x = delta_x * np.cos(curr_yaw) - delta_y * np.sin(curr_yaw)
        global_delta_y = delta_x * np.sin(curr_yaw) + delta_y * np.cos(curr_yaw)

        curr_x += global_delta_x
        curr_y += global_delta_y
        curr_yaw += delta_yaw

        abs_traj.append([curr_x, curr_y, curr_yaw])

    abs_traj_tensor = torch.tensor(abs_traj, dtype=torch.float32)
    return abs_traj_tensor
...
def run_pdm_score_wo_inference(args: List[Dict[str, Union[List[str], DictConfig]]]) -> List[pd.DataFrame]:
    ...
    if cfg.closed_traj_evaluation:
        for token, traj_info in model_trajectory.items():
            rel_traj_list = traj_info['closed_all_trajectory_list']   # 列表中有8个闭环轨迹
            abs_traj = rel2abs_traj_transform(rel_traj_list)
            model_trajectory[token]['trajectory'] = abs_traj
    else:
        # 需要把角度转为弧度制
        for token, traj_info in model_trajectory.items():
            traj = traj_info['trajectory']
            traj[:, 2] = np.deg2rad(traj[:, 2])
            model_trajectory[token]['trajectory'] = traj
            
    tokens_to_evaluate = list(set(scene_loader.tokens) & set(metric_cache_loader.tokens))
    # 仅测试用，使用部分token去评测
    # tokens_to_evaluate = list(set(tokens_to_evaluate) & set(model_trajectory.keys()))
    ...
    for idx, (token) in enumerate(tokens_to_evaluate):
        ...
        try:
            ...
            trajectory = model_trajectory[token]['trajectory'].cpu().numpy()
            # 处理成Trajectory格式
            trajectory = Trajectory(trajectory, TrajectorySampling(time_horizon=4, interval_length=0.5))
...
def main(cfg: DictConfig) -> None:
    ...
    # 直接读取pickle文件,包含traj信息，里面是tensor格式
