...
def add_configured_bev_on_ax_perturbed(ax: plt.Axes, map_api: AbstractMap, frame: Frame, ego_offset: np.ndarray) -> plt.Axes:
    """
    Adds birds-eye-view visualization with perturbation
    :param ax: matplotlib ax object
    :param map_api: nuPlans map interface
    :param frame: navsim frame dataclass
    :param ego_offset: perturbation offset for ego vehicle
    :return: ax with plot
    """
    # Apply ego_offset to the frame's ego_pose
    # perturbed_ego_pose = StateSE2(
    #     frame.ego_status.ego_pose.x + ego_offset[0],
    #     frame.ego_status.ego_pose.y + ego_offset[1],
    #     frame.ego_status.ego_pose.heading + ego_offset[2]
    # )
    ...
    if "annotations" in BEV_PLOT_CONFIG["layers"]:
        add_annotations_to_bev_ax_perturbed(ax, frame.annotations, ego_offset)
        ## 绘制一个原始的ego位置
...
def add_annotations_to_bev_ax_perturbed(ax: plt.Axes, annotations: Annotations, ego_offset: np.ndarray, add_ego: bool = True) -> plt.Axes:
    """
    Adds birds-eye-view visualization of annotations (ie. bounding boxes) with perturbation
    :param ax: matplotlib ax object
    ...
    if add_ego:
        ...
        car_footprint = CarFootprint.build_from_rear_axle(
            rear_axle_pose=StateSE2(0 + ego_offset[0], 0 + ego_offset[1], 0 + ego_offset[2]),
...
def add_trajectory_to_bev_ax_perturbed(ax: plt.Axes, trajectory: Trajectory, config: Dict[str, Any], ego_offset: np.ndarray) -> plt.Axes:
    """
    Add trajectory poses as lint to plot
    :param ax: matplotlib ax object
    :param trajectory: navsim trajectory dataclass
    ...
    :return: ax with plot
    """
    poses = np.concatenate([np.array([[0, 0]]), trajectory.poses[:, :2]]) + ego_offset[:2]
    ax.plot(
        poses[:, 1],
        poses[:, 0],
        color=config["line_color"],
