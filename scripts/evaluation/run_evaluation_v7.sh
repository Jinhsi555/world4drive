...
dir=training_w4d_agent_diff_cmd/navtest_eval_one_stage
...
CKPTS=( $(ls ${ckpt_dir}/epoch*.ckpt 2>/dev/null | sort -V) )
if [ ${#CKPTS[@]} -eq 0 ]; then
...
launch_eval() {
  ...
  (
      ...
      python ${NAVSIM_DEVKIT_ROOT}/navsim/planning/script/run_pdm_score_one_stage_gpu.py \
          ...
          agent.config.model_version=2 \
          agent.config.use_wm=False \
          agent.config.num_mode=18 \
