...
dir=training_w4d_agent_diversity_loss/best_traj_eval
metric_cache_path="${NAVSIM_EXP_ROOT}/metric_cache_${split}_two_stage"
cd ${NAVSIM_DEVKIT_ROOT}
ckpt_dir=/mnt/parallel_ssd/home/zdhs0121/Driving/navsim_workspace/world4drive/exp/training_w4d_agent_diversity_loss/2025.09.27.00.21.59/lightning_logs/version_0/checkpoints
...
launch_eval() {
  ...
  if [ "$SKIP_EXISTING" = "1" ] && [ -f "$subscore_path" ]; then
      echo "[跳过] epoch ${epoch} 已存在结果: $subscore_path"
      return 1
  fi
  ...
  (
      ...
      python ${NAVSIM_DEVKIT_ROOT}/navsim/planning/script/run_pdm_score_gpu_v2_best_traj.py \
          agent=$agent \
